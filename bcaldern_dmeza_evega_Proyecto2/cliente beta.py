...
class Jugador1(pygame.sprite.Sprite):# define la clase Jugador1 la cual recibe como parametro la clase padre pygame.sprite.Sprite
    ...
    def move(self):
        ...
        if pressed[pygame.K_UP]:  # si presiona flecha arriba
            ...
            self.aceleracion += 1 * self.dt  # se le aumentara poco a poco la velocidad de un pixel por segundo
            ...
            if self.velocidad.x > 0:  # y si la velocidad del jugador1 es mayor que cero
                ...
                self.aceleracion -= 1 * self.dt  # que la aceleracion del jugador 1 se le reste un pixel por segundo
            ...
            if abs(self.velocidad.x) > self.dt * self.romper_desaceleracion:  # si el absoluto de la velocidad del jugador 1 x es menor
                ...
                self.aceleracion = -self.velocidad.x / self.dt  # que la aceeracion del jugador1 sea el negativos de la velociddad entre tiempo
        else:  # sino
            if abs(self.velocidad.x) > self.dt * self.desaceleracion_libre:  # si el absoluto de la velocidad del jugador 1 x es menor a la distancia
                # entre tiempo x aceleracion libre
                self.acceleration = -copysign(self.desaceleracion_libre,
                                                  self.velocidad.x)  # que la aceleracion sea la inversion
                # de los valores de velocidad.x y la de romper desaceleracion, negativa
            else:  # sino
                if self.dt != 0:  # si la distancia de la distancia entre tiempo es diferente a 0
                    self.aceleracion = -self.velocidad.x / self.dt  # que la aceleracion de del jugador en x sea la velocidad
                    # en x entre dt
        ...
        if pressed[pygame.K_RIGHT]:  # si se presiona flecha derecha
            self.rotacion -= 30 * self.dt  # que la rotacion del jugador se le reste 30 grados x dt
        elif pressed[pygame.K_LEFT]:  # si se presiona flecha izquierda
            self.rotacion += 30 * self.dt  # que la rotacion del jugador se le sume 30 grados x dt
        else:  # sino
...
class Juego:# se define la clase juego
    ...
    def correr(self):# define el metodo de correr el cual se encarga de darle funcionalidad al programa
        ...
        while not self.exit:# mientras no se use self.exit
            self.dt = self.clock.get_time() / 1000 # la distancia entre tiempo sera de los microsengundos divididos entre mil para definir el tiempo de movimiento
            self.dt2 = self.clock.get_time() / 1000# la distancia entre tiempo sera de los microsengundos divididos entre mil para definir el tiemp en movimiento
