...
class Arbol(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image=pygame.image.load("imagenes proyercto 2/ Arbol.png")
        self.rect=self.image.get_rect()

class minas(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image=pygame.image.load("imagenes proyercto 2/ Mina.png")
        self.rect=self.image.get_rect()
...
class Jugador1(pygame.sprite.Sprite):
    def __init__(self, x=220, y=150, angulo=0.0, largoVehiculo=3, maxima_rotacion=30, maxima_aceleracion=200.0):
        super().__init__()
        self.imagen = pygame.image.load("imagenes proyercto 2/ enemies_level1.png")
...
class Jugador2(pygame.sprite.Sprite):
    def __init__(self, x=130, y=95, angulo=0.0, largoVehiculo=3, maxima_rotacion=30, maxima_aceleracion=200.0):
        super().__init__()
        self.posicion = Vector2(x, y)
        self.velocidad = Vector2(0.0, 0.0)
        self.image = pygame.image.load("imagenes proyercto 2/ enemies2_level1.png")
        self.rect = self.image.get_rect()
        self.rect.center = (x,y)
        self.angulo = angulo
        self.largoVehiculo = largoVehiculo
 ...
 #actualiza el jugador2
    def update(self, dt2):
        self.velocidad += (self.aceleracion * dt2, 0)
        self.velocidad.x = max(-self.maxima_velocidad, min(self.velocidad.x, self.maxima_velocidad))
        self.rect.x=self.posicion.x
        self.rect.y=self.posicion.y
        if self.rotacion:
        ...
        self.posicion += self.velocidad.rotate(-self.angulo) * dt2
        self.angulo += degrees(velocidad_angular) * dt2
...
class Juego:
    def __init__(self):
        ...
        self.ticks = 60
        self.exit = False
        self.pista1=pygame.image.load("imagenes proyercto 2/ modelo fondo.png")
        def level1(self):
            jugador1 = Jugador1()
            jugador2 = Jugador2()
            Meta = meta()
            ...
            for catus in range(12):
                arbol.rect.x= random.randrange(ALTURA)
                arbol.rect.y = random.randrange(ANCHO)
            ...
            carros.add(jugador1)
            carros2.
            carros2.add(jugador2)
            choque = pygame.sprite.spritecollide(arbol, carros, True)
            if choque:
